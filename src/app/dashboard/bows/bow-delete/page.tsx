...
export default function BowDeletePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (res.status === 401) {
          location.href = "/login?next=/dashboard/bows/bow-delete";
          return;
        }
  ...
  const handleDelete = async (targetId: string) => {
    if (!targetId) return;
    const confirmed = window.confirm("この弓を削除しますか？");
    if (!confirmed) return;
    setDeletingId(targetId);
    setMessage("");
    try {
      const res = await fetch("/api/bows/delete", {
        method: "POST",
      ...
      if (!res.ok) {
        throw new Error(data.error || "弓の削除に失敗しました");
      }
      setMessage("削除しました");
      setList((prev) =>
        prev.status === "loaded"
          ? {
              status: "loaded",
              bows: prev.bows.filter((bow) => bow.id !== targetId),
            }
          : prev
      );
    } catch (err) {
              ...
              <div key={bow.id} className="card">
                ...
                <div className="mt-4 inline-list">
                  <button
                    className="btn btn-primary"
                    type="button"
                    onClick={() => void handleDelete(bow.id)}
                    disabled={deletingId === bow.id}
                  >
                    <span className="inline-flex items-center gap-2">
                      <HiOutlineTrash className="text-base" />
                      {deletingId === bow.id ? "削除中..." : "削除"}
                    </span>
                  </button>
                </div>
              </div>
