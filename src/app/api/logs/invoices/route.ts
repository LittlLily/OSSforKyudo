...
export async function GET() {
  ...
  try {
    ...
    const profileIds = Array.from(
      new Set(
        logs
          .flatMap((log) => [log.operator_id, log.subject_user_id])
          .filter(Boolean)
      )
    );
    ...
    const payload = logs.map((log) => ({
      ...
      operator_student_number:
        log.operator_id && profilesById.get(log.operator_id)
          ...
          : null,
      target_id: log.subject_user_id,
      target_display_name:
        log.subject_user_id && profilesById.get(log.subject_user_id)
          ? profilesById.get(log.subject_user_id)?.display_name ?? null
          : null,
      target_student_number:
        log.subject_user_id && profilesById.get(log.subject_user_id)
          ? profilesById.get(log.subject_user_id)?.student_number ?? null
          : null,
